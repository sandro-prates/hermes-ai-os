...
def test_real_project_state_has_explicit_active_completed_and_planned_values() -> None:
    ...
    assert active["sprint"] == {
        "id": "SPRINT-11",
        "title": "Container Artifact Publication Baseline",
        "status": "in_progress",
    }
    ...
    activation = state["sprint_11_activation_baseline"]
    assert isinstance(activation, dict)
    assert activation["package_visibility"] == "PRIVATE"
    assert activation["public_visibility_authorized"] is False
    assert activation["technical_implementation_authorized"] is False
    assert activation["package_publication_authorized"] is False
